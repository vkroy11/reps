...
export function generateContentPrompt(input: GenerateContentInput): string {
  ...
  return [
    ...shared,
    'Write a micro-lesson in markdown: just enough to do the rep above.',
    'No history, no theory the rep does not need. Then 2-5 key points.',
    '',
    'Markdown rules the reader actually renders:',
    '- **bold** for names, *italic* for labels, `code` for identifiers.',
    '- Every code sample MUST be a fenced block: ```python ... ``` (or js, ts, go).',
    '  Never a bare "python" line followed by indented code.',
    '- If the skill is programming, algorithms, syntax or data structures:',
    '  include one ```mermaid flowchart TD``` of the idea (under 8 nodes, no styling)',
    '  AND one short complete snippet in a fenced block. A wall of prose is not enough.',
    '',
